"""Converts IDD Lite data to TFRecord file format with Example protos."""
...
def _convert_dataset(dataset_split, dataset_dir, dataset_label_dir):
  """Converts the IDD Lite dataset into into tfrecord format.
  ...
  """
  imgloc = os.path.join(dataset_dir, '*/*.jpg')
  img_names = tf.gfile.Glob(imgloc)
  random.shuffle(img_names)
  seg_names = []
  for f in img_names:
    # get the filename without the extension
    basename = '/'.join(f.split('/')[-2:]).split('.')[0].replace('_image', '_label')
